//  Most of this shader has been taken from "Simplex.metal".
/// Part of [AHNoise](https://github.com/AndyHeardApps/AHNoise).
//  Created by Andrew Heard on 23/02/2016.
//  Copyright © 2016 Andrew Heard. All rights reserved.
//  Distributed under the [MIT licence](https://github.com/AndyHeardApps/AHNoise/issues/1).
...
constant vec4 keepThisHereToAvoidCompilerBug = vec4(1.0f, 1.0f, 1.0f, 1.0f);
...
float calculateHeight(vec4 p, float radius, float delta, float time) {
  ...
  if (delta < 0.0f) {
    ...
    height = radius + 0.005f * delta * sin(5.0 * (time + s));
  }
  return height;
}
...
float offset = 1.0f;
float hL = calculateHeightForPosition(p + vec4(offset, 0.0, 0.0, 1.0));
float hR = calculateHeightForPosition(p + vec4(-offset, 0.0, 0.0, 1.0));
float hD = calculateHeightForPosition(p + vec4(0.0, offset, 0.0, 1.0));
float hU = calculateHeightForPosition(p + vec4(0.0, -offset, 0.0, 1.0));
